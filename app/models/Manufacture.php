<?php 
    ...
    class Manufacture extends Controller{
        ...
        public function add($man_name,$man_user,$description, $brand, $image){
            $this->db->query("INSERT INTO 
            manufactures (man_name,man_user,active,description,brand,image)
            VALUES (:man_name,:man_user,0,:description, :brand, :image)");
        ...
        public function update($id, $name, $description, $brand, $image)
        {
            $this->db->query("UPDATE manufactures SET man_name = :man_name, description = :description, brand = :brand, image = :image WHERE man_id = :man_id");
            $this->db->bind(':man_name', $name);
            $this->db->bind(':description', $description);
            $this->db->bind(':brand', $brand);
            $this->db->bind(':image', $image);
